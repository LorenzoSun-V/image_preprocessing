...
Author: shy
Description: 
LastEditTime: 2021-09-27 13:54:06
'''
import os
import cv2
import requests
...
class SoftFPN():
	...
	def __call__(self, detect_data, frame_id=None):
		...
		# 如果多张图片的numpy数组组成的列表数据
		if isinstance(detect_data, list):
			# img_name = None
			# # 将numpy数组转换为列表
			# frame_list = [data.tolist() for data in detect_data]
			# frame_data = json.dumps(frame_list)
		...
		# 如果是一张图片的numpy数组数据
			...
			raise NotImplementedError(
				"Unknown input type {}".format(type(detect_data))
			)
